...
app.get('/punched', (req, res) => {
    ...
    res.end(JSON.stringify({total: phoners.length, phones: phoners}));
...
app.post('/update', (req, res) => {
    ...
    if (!(!!req.body.name && !!req.body.type && !!req.body.expire)) {
        res.status(500).send("Missing property").end();
        return;
    }
    ...
    if (isNaN(expireTime)) {
        res.status(500).send("Invalid date/time").end();
        return;
    }
    else {
    ...
    if (validTypes.indexOf(breakType) === -1) {
        res.status(500).send("Invalid break type").end();
        return;
    }

    if (!(slack.getUser(username) instanceof Object)) {
        res.status(500).send("Invalid username").end();
        return;
    }
