...
function upkeepOnBreak() {
    return new Promise(function (fulfill, reject) {
        ...
        for (var user in breaks.onbreak) {
            ...
            /* break expired */
            if (delta > 0) {
                //breaks.onbreak[user].timerobj = setInterval(function () { console.log('INTERVAL'); sendReminder(user); }, conf_breaks.remindTime * 1000);
        ...
        requests.getAgentStatus(user)
            .then(function (res) {
                ...
                if (res === 'accepting chats' || res === 'offline') {
                    delete breaks.out[user];
                    console.log(user + ' silently changed status to: ' + res + '. Deleting metadata.');
                }
