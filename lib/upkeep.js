...
function upkeepCooldown(channel) {
    return new Promise(function (fulfill, reject) {
        ...
        let coolList = channel.meta.cooldown;
...
function sendReminder(user, channel, timeOver) {
    ...
    let superMessage,
        ...
        breakType = !!channel.breaks.over[user].breakType ? channel.breaks.over[user].breakType : 'brb',
        outTime = new Date(channel.breaks.over[user].outTime).toLocaleTimeString();

    // console.log(breakType,outTime);
    superMessage = `\`#${channel.name}\`: ${user} is *${timeOver}* minutes over their ${breakType} break (STARTED: ${outTime})`;

    slack.sendPrivateMessage(user + ': you need to log back into #' + channel.name + ' with *!back*', userID);

    //supers.notifySupers('`' + user + '`' + ' is *' + timeOver + 'm* over their break.', user, channel.name);
    supers.notifySupers(superMessage, user, channel.name);
}
...
function clearBreakLimits() {
    for (let channel in globals.channels) {
        ...
        globals.channels[channel].meta.count = {};
    }
}
