...
var conf_breaks = require('../conf/config.breaks');
...
function upkeep() {
    ...
    for (let channel in globals) {
        ...
        // if (Math.floor(process.uptime()) % 30 === 0 && conf.ENV !== 'dev')
        //     checkConcurrent();
...
function upkeepOnBreak() {
    return new Promise(function (fulfill, reject) {
        ...
        for (let user in breaks.active) {
            ...
            var delta = (now - (breaks.active[user].outTime + (breaks.active[user].duration  * 1000))) / 60000;

            /* break expired */
            if (delta > 0) {
                breakExpired(user, breaks.active);
            }
...
function upkeepOverBreak() {
    ...
    for (var user in breaks.overbreak) {
        ...
        var delta = (now - (breaks.over[user].outTime + (breaks.over[user].duration * 1000))) / 1000;
...
function breakExpired(user, list) {
    ...
    //TODO
    //check if user logged back in without the bot
    slack.sendMessage(user + ': your break has expired.', breaks.over[user].channel);
    delete list[user];
    ...
    //             delete list[user];
    //         }
    //         else if (status === 'accepting chats' || status === 'offline') {
    //             breaks.clearBreaks(user);
    //             console.log(new Date().toLocaleString() + ' ' + user + ' silently changed status to: ' + status + '. deleting break data. (FROM: breakExpired)');
    //         }
    //     })
    //     .catch(function (err) {
    //         console.error(err);
 ...
 */
function sendReminder(user, channel) {

    //TODO
    //check if user logged back in before sending reminder
    slack.sendMessage(user + ': you need to log back into chats with *!back*', channel);
    ...
    //
    //         if (status === 'not accepting chats') {
    //             slack.sendMessage(user + ': you need to log back into chats with *!back*', breaks.over[user].channel);
    //         }
    //
    //         else {
    //             breaks.clearBreaks(user);
    //             console.log(new Date().toLocaleString() + ' ' + user + ' silently changed status to: ' + status + '. deleting break data. (FROM: sendReminder)');
    //         }
    //     })
    //     .catch(function (err) {
...
function checkBounces() {
    /*
     * don't think this will be relevant again
     */
...
function checkLunch(channel) {
    ...
    if (!(channel.hasOwnProperty('schedule'))) {
        console.error('BAD LUNCH OBJECT, SKIPPING:');
    ...
    for (let user in channel.schedule) {
        ...
        if (!channel.schedule[user].notified && (channel.schedule[user].time - 15 * 1000) <= now) {
            slack.sendMessage(user + ': get ready, lunch in 15!', channel.id);
            channel.schedule[user].notified = 1;
        }

        //send reminder when lunch time rolls around
        else if ((channel.schedule[user].time.getHours() % 12) === (now.getHours() % 12) && channel.schedule[user].time.getMinutes() === now.getMinutes()) {
            slack.sendMessage(user + ': time for lunch! Use *!lunch* when you\'re ready!', channel.id);
        }

        //expire old lunches
        if (channel.schedule[user].time.getTime() + conf_breaks.lunchExpire * 1000 <= now.getTime()) {
            console.log('lunch expired: ' + user + ', ' + channel.schedule[user].time);
            delete channel.schedule[user];
        }
    }
}
...
//this whole thing should probably be removed
    ...
    Object.keys(wrapup.concurrent).forEach(function (chatter) {
        ...
        //check timer, log in when wrapup is over
            ...
            if (chatterobj.start.getTime() + conf_breaks.wrapupTime * 1000 < now.getTime()) {
                ...
                if (!(breaks.active.hasOwnProperty(chatterobj.user) ||
                    breaks.over.hasOwnProperty(chatterobj.user) ||
