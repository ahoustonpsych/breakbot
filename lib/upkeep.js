...
module.exports = {
    ...
    getPunchedInSupers: getPunchedInSupers
};
 ...
 */
function notifySupers(msg, user, channel) {
    ...
    list.forEach((supervisor) => {
        console.log(new Date().toLocaleString(),'SENT REMINDER TO:', supervisor.name, 'FOR:', user, '#' + channel);
        //TODO
...
function getSupervisorByEmployee(username) {
    return new Promise((fulfill,reject) => {
        ...
        db.db.query(query, (err, userSuper) => {
            ...
            // {
            //     console.log('supervisor:', userSuper[0].supervisor);
            //     getUnderlingsByName(userSuper[0].supervisor)
            //         .then(res => {
            //             if (!res) reject('no underlings');
            //             console.log('underlings:', res)
            //             punched = getPunchedInSupers(res);
            //             console.log(typeof punched, punched.length)
            //             if (!punched[0]) reject('no punched-in supers');
            //             else {
            //                 console.log('punched in supers:', punched)
            //                 fulfill('success!');
            //             }
            //         })
            //         .catch(err => {
...
//TODO
function getPunchedInSupers(list) {
    ...
    if (punched.length === 0) punched.push('none found');
    return punched;

    // let userSuper,
    //     allSupers = channel.supervisors;
    //
    // userSuper = getSupervisorByEmployee(username);
    //
    // if (!userSuper) { console.log('no supervisor'); return }
    //
    // if (!!channel.punched ||
    //     !!channel.punched[username] ||
    //     !!channel.punched[username].supervisor) {
    //     console.error(new Date().toLocaleString(), 'NO SUPERVISOR FOUND FOR:', username);
    //     return [];
    // } else {
    //     return channel.punched[username].supervisor;
    // }

    //let punchedSupers = []

    // _.forEach(allSupers, (superObj) => {
    //     if (!!superObj || !!superObj.username || !!superObj.supervisor)
    //         return [];
    //
    // })
}
