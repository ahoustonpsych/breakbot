...
function upkeep() {
    ...
    if (Math.floor(process.uptime()) % 58 === 0)
        checkLunch();
...
function checkLunch() {
    ...
    for (var user in luncher.schedule) {
        ...
        if (!luncher.schedule[user].notified && (luncher.schedule[user].time - 15  * 1000) <= now) {
        ...
        //expire old lunches
        if (luncher.schedule[user].time.getTime() + conf_breaks.lunchExpire  * 1000 <= now.getTime()) {
