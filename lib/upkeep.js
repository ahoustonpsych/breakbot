...
function upkeepBio(channel) {
    ...
    for (let user in breaks.bio) {
        ...
        /* minutes remaining in lunch */
        breaks.bio[user].remaining = Math.floor(Math.abs((now - (breaks.bio[user].outTime + (breaks.bio[user].duration * 1000))) / 60000));

        /* now - (time of break + duration of break) in seconds */
        let delta = (now - (breaks.bio[user].outTime + (breaks.bio[user].duration * 1000))) / 60000;
...
function checkLunch(channel) {
    return new Promise(function (fulfill, reject) {
        ...
        for (let user in channel.schedule) {
            ...
            let userId = slack.dataStore.getUserByName(user).id;
            console.log(channel.schedule[user])
            console.log(userId)
