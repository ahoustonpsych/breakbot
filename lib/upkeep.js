    ...
    for (let channel in globals) {
        ...
        upkeepOnBreak(globals[channel])
            .then(function () {
                ...
                //
                //console.log(globals[channel].breaks)
...
function upkeepOnBreak(channel) {
    return new Promise(function (fulfill, reject) {
        ...
        for (let user in breaks.active) {
            ...
            breaks.active[user].remaining = Math.floor(Math.abs((now - (breaks.active[user].outTime + (breaks.active[user].duration * 1000))) / 60000));

            /* now - (time of break + duration of break) in seconds */
            //debug. time until break expires
            let delta = (now - (breaks.active[user].outTime + (breaks.active[user].duration * 1000))) / 60000;
...
function breakExpired(user, list, channel) {
    ...
    slack.sendPrivateMessage(user + ': your break has expired.', userID);
