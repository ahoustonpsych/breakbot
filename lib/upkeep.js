...
function upkeep() {
    ...
    for (let channel in globals.channels) {
        ...
        if (!(currentChannel instanceof Object)) {
            console.error(new Date().toLocaleString() + ' ^ BAD CHANNEL OBJECT, SKIPPING:');
            console.error(currentChannel);
            continue;
        }

        upkeepCooldown(currentChannel)
            .then(() => {
                /* process active breaks */
                Promise.all([
                    upkeepOnBreak(currentChannel),
                    upkeepTask(currentChannel),
                    upkeepBio(currentChannel),
                    upkeepLunch(currentChannel)
                ])
                    .then(function (vals) {
                        upkeepOverBreak(currentChannel);
                        upkeepSchedule(currentChannel);
