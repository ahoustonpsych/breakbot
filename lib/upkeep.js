            ...
            .then(() => {
                ...
                Promise.all([
                    ...
                    .then(function (vals) {
                        ...
                        checkLunch(globals.channels[channel]);
...
function checkLunch(channel) {
    return new Promise(function (fulfill, reject) {
        ...
        for (let time in channel.schedule) {
            // console.log('checking:');
            // console.log(user);
            if (!(channel.schedule[time].time instanceof Date)) {
                console.error(new Date().toLocaleString() + ' INVALID LUNCH TIME, SKIPPING:');
                console.error(channel.schedule[time]);
                delete channel.schedule[time];
                continue;
            }
            ...
            if (!channel.schedule[user].notified && (channel.schedule[user].time - 15 * 60 * 1000) <= now) {
