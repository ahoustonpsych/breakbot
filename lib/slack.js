...
let rtm = new RtmClient(config.slackAPIKey, {
    ...
    dataStore: false    //new MemoryDataStore()
});
...
rtm.getUser = function (ident) {
    ...
    return rtm.users.info(ident).user || 'not defined';     // TODO: currently only supports user IDs
};
...
rtm.getChannel = function (ident) {
    ...
    return rtm.channels.info(ident) || 'not defined';   // TODO: currently only supports channel IDs
};
