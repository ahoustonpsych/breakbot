...
function arrangePunchedIn(deptData, punches) {
    ...
    Object.keys(punches).forEach((user) => {
        if (Object.keys(deptData).indexOf(user) !== -1) {
            ...
            validUsers[user].position = deptData[user].position;
        }
    });

    console.log('chatters:')
    console.log(topic.getChatters('breakbot-livechat'));

    Object.keys(validUsers).forEach((name) => {
        let dept = validUsers[name].department;
        //console.log(dept);

        if (!(globals.hasOwnProperty(conf.channelDesignation[dept])))
            return;

        //console.log(conf.channelDesignation[dept]);
        let numChatters = topic.getChatters('breakbot-livechat').length;

        //set chatter break limits
        globals[conf.channelDesignation['LiveChat']].punchCount = numChatters;



        globals[conf.channelDesignation['LiveChat']].maxOnBreak = Math.ceil(numChatters * 0.15);
        //log all users that are punched in
        globals[conf.channelDesignation[dept]].punches[name] = validUsers[name];
        //total punched in
        globals[conf.channelDesignation[dept]].punchCount = Object.keys(globals[conf.channelDesignation[dept]].punches).length - numChatters;

        //sets maxOnBreak based on the number of people punched in
        globals[conf.channelDesignation[dept]].maxOnBreak = Math.ceil(globals[conf.channelDesignation[dept]].punchCount * 0.15);

    });
    // console.log(globals['breakbot-support'].punches);
    // console.log('num: ' + globals['breakbot-support'].punchCount);

    //setLCBreakLimit();

}

function setLCBreakLimit() {

    let numChatters = topic.getChatters('breakbot-livechat').length
    let numPunched = globals[conf.channelDesignation['Support']].punchCount;

    console.log(topic.getChatters('breakbot-livechat').length);
