    ...
    getAgentData()
        .then((punchList) => {
            cleanPunchList(punchList)
                ...
                .catch((err) => console.error(new Date().toLocaleString(), 'ERROR READING PUNCH DATA:', err));
        })
...
function getAgentData() {
    return new Promise((fulfill, reject) => {
        apiCall(conf.wallboardURL[conf.ENV], '/api/data/agents')
...
function cleanPunchList(list) {
    return new Promise(function (fulfill, reject) {
        ...
        if (!!slack) reject("slack hasn't connected yet, skipping punches for now");
...
function logPunchData(agentdata) {
    ...
    check(agentdata)
        ...
        .catch(err => {
            //console.error(new Date().toLocaleString(), 'ERROR QUERYING DATABASE:', err)
            });
