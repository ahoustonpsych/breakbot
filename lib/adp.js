...
function arrangePunchedIn(punched) {
    ...
    Object.keys(punched).forEach((name) => {
        ...
        globals.channels[conf.channelDesignation['livechat']].maxOnBreak =
            Math.ceil(numChatters * conf_breaks.maxOnBreakPercentageLiveChat);

        /* set minimum on-break limit */
        if (globals.channels[conf.channelDesignation['livechat']].maxOnBreak < 2)
            globals.channels[conf.channelDesignation['livechat']].maxOnBreak = 2;

        //log all users that are punched in
        globals.channels[conf.channelDesignation[dept]].punches[name] = punched[name];

        //total punched in
        globals.channels[conf.channelDesignation[dept]].punchCount =
            Object.keys(globals.channels[conf.channelDesignation[dept]].punches).length - numChatters;

        //sets maxOnBreak based on the number of people punched in
        globals.channels[conf.channelDesignation[dept]].maxOnBreak =
            Math.ceil(globals.channels[conf.channelDesignation[dept]].punchCount * conf_breaks.maxOnBreakPercentage);

        if (globals.channels[conf.channelDesignation[dept]].maxOnBreak < 2)
            globals.channels[conf.channelDesignation[dept]].maxOnBreak = 2;
...
function logPunchData(data) {
    ...
    //console.log(logdata);
    db.log('punch_history_test', logdata)
        .catch((err) => {
            if (err.code !== 'ER_DUP_ENTRY')
                console.error(new Date().toLocaleString(), 'ERROR ADDING PUNCH DATA TO DB', err);
        })
